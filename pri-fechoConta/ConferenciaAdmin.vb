...
Public Class ConferenciaAdmin
    ...
    Public Function adicionarCDUfacNoDocumentosBancos() As Boolean

        Dim query As String = "ALTER TABLE dbo.DocumentosBancos ADD CDU_fac bit"
    ...
    Public Sub insertLicenca(serie As String, numeroUtilizador As Integer, dataInicio As String, dataFim As String)
        Try
            ...
            If (SQL.temConexao()) Then
                Dim query As String = "INSERT INTO TDU_ConferenciaLicenca (CDU_serie_licenca , CDU_numero_utilizador, CDU_dataLicenca , CDU_dataLicencaFim ) VALUES ('" & serie & "',  '" & numeroUtilizador & "', Convert(Date, '" & dataInicio & "', 105), Convert(Date, '" & dataFim & "', 105))"
                SQL.abrirCon()
    ...
    Public Function insertConferenciaCaixa(caixa As Long, movimento As String, cartaoTipo As String, transacaoNumero As String, quantidade As Integer, valor As Double, chequeNumero As String, chequeDescricao As String, dataConferencia As String, tesoureiro As String, saidaDescricao As String, referencia As String, idCabecTesourario As String, contaPos As String, data_fecho As String, data_transacao As String, rec_inicial As Integer, rec_final As Integer, rec_serie As String, Optional utilizadorCaixa As String = "")

        Try

            If (SQL.temConexao()) Then
                Dim query As String = "INSERT INTO TDU_ConferenciaCaixa ( CDU_diarioCaixa , CDU_modoMovimento , CDU_cartaoTipo , CDU_TransacaoNumero, CDU_quantidade , CDU_valor , CDU_chequeNumero , CDU_chequeDescricao , CDU_dataConferencia , CDU_utilizadorTesoureiro , CDU_saidaDescricao , CDU_referencia, CDU_IdCabecTesouraria, CDU_conta, CDU_data_fecho, CDU_data_transacao, CDU_rec_num_inicial, CDU_rec_num_final, CDU_rec_serie, CDU_utilizadorCXMT) " &
                    "VALUES ('" & caixa & "',  '" & movimento & "', '" & cartaoTipo & "','" & transacaoNumero & "', '" & quantidade & "',  '" & valor & "',  '" & chequeNumero & "',  '" & chequeDescricao & "',  '" & dataConferencia & "',  '" & tesoureiro & "',  '" & saidaDescricao & "',  '" & referencia & "', '" & idCabecTesourario & "', '" & contaPos & "', '" & data_fecho & "' ,'" & data_transacao & "', '" & rec_inicial & "', '" & rec_final & "', '" & rec_serie & "'  , '" & utilizadorCaixa & "' )"
                SQL.abrirCon()
    ...
    Public Function removeLinhasCaixa(diarioCaixa As Long, conta As String, data As String, rec_inicial As Integer, rec_final As Integer, rec_serie As String, Optional utilizadorCaixa As String = Nothing)
        Dim query As String
        Dim rv As Boolean = False
        Try
            ...
            If (SQL.temConexao()) Then
                ...
                If (conta = "CXMT") Then
                    If (utilizadorCaixa <> Nothing) Then
                        'query = "delete from TDU_ConferenciaCaixa where CDU_conta = '" & conta & "' and CDU_data_fecho = '" & data & "'  and CDU_rec_num_inicial = '" & rec_inicial & "' and CDU_rec_num_final = '" & rec_final & "'   and CDU_rec_serie = '" & rec_serie & "' and CDU_utilizadorCXMT = '" & utilizadorCaixa & "' "

                        query = "delete from TDU_ConferenciaCaixa where CDU_conta = '" & conta & "' and CDU_data_fecho = '" & data & "'   and CDU_rec_serie = '" & rec_serie & "' and CDU_utilizadorCXMT = '" & utilizadorCaixa & "' "
                    Else
    ...
    Public Function buscarUtilizador() As List(Of String)
        ...
        tabela = new sqlControloPriEmpre().buscarDado(query)
        If (tabela.Rows.Count <> 0) Then
            For i As Integer = 0 To tabela.Rows.Count - 1
                listaSerie.Add(tabela.Rows(i)("Codigo"))
            Next
        End If
